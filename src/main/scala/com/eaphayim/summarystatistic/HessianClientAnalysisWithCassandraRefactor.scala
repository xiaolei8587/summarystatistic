 ...
 * @version 2015-01-15
 */
object HessianClientAnalysisWithCassandraRefactor extends Logging {

  def main(args: Array[String]): Unit = {
    if (args.length < 3) {
      throw new SparkException("args need params:host port batchInterval")
    }
    val pattern = "\\D".r
    ...
    val checkpointDirectory = "hdfs://mucfc/in/flume"
    val ssc = StreamingContext.getOrCreate(checkpointDirectory,
      () => {
        createContext(hostStr, portStr, interval, checkpointDirectory)
      })
    ssc.start()
    ssc.awaitTermination()
  }

  def createContext(hostStr: String, portStr: String, interval: String, checkpointDirectory: String) = {
    ...
    val summaryDateWindow = "analysissummary_date_win"
    ...
    CassandraConnector(conf).withSessionDo { session =>
      ...
      session.execute(s"CREATE TABLE IF NOT EXISTS $keyspace.$summaryDateWindow (appname VARCHAR , date VARCHAR, methodname VARCHAR, costs INT, success INT, total INT, PRIMARY KEY(appname,date,methodname) )")
    }
    ...
    lazy val updateFunc = (values: Seq[LogAnalysisSummary], state: Option[LogAnalysisSummary]) => {
      ...
      if (previousCount != null) {
        println("********previousCount :" + previousCount.methodname + "," + previousCount.total + "-----" + summary+"---"+values.size)
      } else {
        println("********previousCount :null-----" + summary+"---"+values.size)
      }
    ...
    summaryDateStream.checkpoint(Seconds(10))
    ...
    val windowStream = summaryDateStream.reduceByKeyAndWindow((a: LogAnalysisSummary, b: LogAnalysisSummary) => (a.count(b)), (a: LogAnalysisSummary, b: LogAnalysisSummary) => (a.minus(b)), Seconds(8), Seconds(10))
    //    val windowStream = summaryDateStream.reduceByKeyAndWindow((a,b)=>reduceByWindow(a,b), Seconds(10), Seconds(10))
    windowStream.checkpoint(Seconds(10))
    val saveSummaryWindowStream = windowStream.updateStateByKey[LogAnalysisSummary](updateFunc).map(data => data._2)
    saveSummaryWindowStream.saveToCassandra(keyspace, summaryDateWindow, SomeColumns("appname", "date", "methodname", "costs", "success", "total"))
    ssc.checkpoint(checkpointDirectory)
    ssc
  }

  def reduceByWindow(a: LogAnalysisSummary, b: LogAnalysisSummary): (LogAnalysisSummary) = {
    val beginTime = System.currentTimeMillis()
    val str = "********reduceByWindow********:%s,%s at time:%s".format(a, b, beginTime)
    println(str)
    logInfo(str)
    (a.count(b))
  }
  ...
  def getLogAnalysisSummary(data: LogAnalysisSummary): LogAnalysisSummary = {
    ...
    println("********getLogAnalysisSummary costs:" + costs + "---" + data.methodname + "," + data.total + "--" + obj.total)
    return obj
  }
  ...
  def saveData(rdd: RDD[LogAnalysisSummary]): Unit = {
    ...
    val size = arr.size
    if (size > 0) {
      println("********saveData collect:" + arr.size)
      var num = 100;
      while (size < num) {
        num = num / 2
      }
      ...
      val endTime = System.currentTimeMillis()
      val costs = endTime - beginTime
      println("********all sql execute costs:" + costs)
    }
 ...
 * @param preStatement
 */
class DataHandler(arr: Array[LogAnalysisSummary], session: com.datastax.driver.core.Session, preStatement: PreparedStatement) extends Callable[String] with Logging {
  override def call(): String = {
    ...
    session.execute(sb.toString)
    val endTime = System.currentTimeMillis()
    "sql costs:" + (endTime - beginTime) + "----" + arr.size
  }
}